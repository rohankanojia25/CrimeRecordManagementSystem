...
<div class="container2" >
  ...
  $sql = "SELECT ID, firstname, lastname, city, crime, charges, aadhaar, gender FROM criminalreg";
  $result = $conn->query($sql); 
  if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
  echo "<tr><td>" . $row["ID"]. "</td><td>" . $row["firstname"]. "</td><td>" .$row["lastname"]. "</td><td>" .$row["city"]. "</td><td>" .$row["crime"]. "</td><td>" .$row["charges"]. "</td><td>" .$row["aadhaar"]. "</td><td>" .$row["gender"]. "</td></tr>";
  }
  echo "</table>";
  } else { echo "0 results"; }
  $conn->close();
